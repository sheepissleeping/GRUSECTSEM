...
class Solver(object):
    def __init__(self, args):
        ...
        if len(self.args.gpu_ids) > 1:                                                #根据 GPU 数量进行多卡并行
        ...
        # print(f">>> [Debug] len(train_dataloader) = {len(self.train_dataloader)}")

        self.test_dataloader = create_dataloader(self.args, "test")

        # self.infer_dataloader = create_dataloader(self.args, "infer")

        # init criterion
        self.criterion = nn.CrossEntropyLoss(                             #使用带标签平滑的交叉熵损失
        ...
        checkpoint = torch.load('/home/menglingping/TFace-master/attribute/M3DFEL/outputs/DFEW-[09-04]-[16:39]/model_best.pth', map_location='cuda:0')
        state_dict = {}
        ...
        # resume
        if args.resume:                                                              #若指定断点，则加载模型和优化器状态。
    ...
    def run(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):                  #（0，epochs)
            inf = '********************' + str(epoch) + '********************'
            start_time = time.time()

            with open(self.log_path, 'a') as f:                                    #保存并打印当前 epoch 的日志头
                f.write(inf + '\n')
            print(inf)
            ...
            is_best = (val_acc[0] > self.best_wa) or (                         #检查是否为最佳模型，保存状态
            ...
            # if is_best:                                                #如果是当前最优模型，绘制并保存混淆矩阵热图
    ...
    def validate(self, epoch):
        ...
        for i, (images, target) in enumerate(self.test_dataloader):
            ...
            #loss = self.criterion(output, target)
            loss = self.criterion(output.unsqueeze(0) if len(output.shape)==1 else output, target)

            # pred = torch.argmax(output, 1).cpu().detach().numpy()
            if output.dim() == 1:
                pred = torch.argmax(output.unsqueeze(0), 1).cpu().detach().numpy()
            else:
                pred = torch.argmax(output, 1).cpu().detach().numpy()
            target = target.cpu().numpy()
